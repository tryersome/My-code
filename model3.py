...
class Encoder_rgb1(nn.Module):
    def __init__(self):
        super(Encoder_rgb1,self).__init__()
        resnet1 = models.resnet50(pretrained=True)
        self.encoder0 = nn.Sequential(resnet1.conv1, resnet1.bn1, resnet1.relu)
    def forward(self,tensor):
        res0 = self.encoder0(tensor)  #[64, 240, 320]
...
class Encoder_t1(nn.Module):
    def __init__(self):
        super(Encoder_t1,self).__init__()
        resnet1 = models.resnet50(pretrained=True)
        self.encoder0 = nn.Sequential(resnet1.conv1, resnet1.bn1, resnet1.relu)
    def forward(self,tensor):
        res0 = self.encoder0(tensor)  #[64, 240, 320]
...
class Encoder_rgbt(nn.Module):
    def __init__(self):
        super(Encoder_rgbt,self).__init__()
        resnet1 = models.resnet50(pretrained=True)
        self.encoder1 = nn.Sequential(resnet1.maxpool, resnet1.layer1)
        self.encoder2 = resnet1.layer2
        self.encoder3 = resnet1.layer3
    ...
    def forward(self,tensor):
        res1 = self.encoder1(tensor)   #[256, 120, 160]
        res2 = self.encoder2(res1)   #[512, 60, 80]
        res3 = self.encoder3(res2)   #[1024, 30, 40] 
...
class Encoder_Block(nn.Module):
    def __init__(self):
        ...
        self.channel_atten0=ChannelAttention(1024,16)     #64
        self.channel_atten1=ChannelAttention(1024,4)      #256
...
class TFNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.Encoder_rgb1 = Encoder_rgb1()
        self.Encoder_t1 = Encoder_t1()
        self.Encoder_rgbt= Encoder_rgbt()
        self.Encoder_rgb= Encoder()
        self.Encoder_t= Encoder()

        self.fuse_conv1=ConvBnrelu2d_3(64+64, 64)
        self.fuse_conv2=ConvBnrelu2d_3(64+3, 64)
        self.fuse_conv3=ConvBnrelu2d_3(64+3, 64)
    ...
    def forward(self, input_rgb,input_t):           
        ...
        r0=self.Encoder_rgb1(input_rgb)      
        t0=self.Encoder_t1(input_t)        

        feature0=torch.cat((r0,t0),dim=1)      
        feature0=self.fuse_conv1(feature0)    

        rt1,rt2,rt3=self.Encoder_rgbt(feature0)
        ...
        fusion0,fusion1,fusion2,fusion3,fusion_loss=self.Decoder_RGBT(p0,p1,p2,p3)   #第一段解码     
        # 512 60 80    256 120 160   64 240 320   64 480 640   9 480 640  

        rgb_com=torch.cat((input_rgb,fusion3),dim=1)
        rgb_com=self.fuse_conv2(rgb_com)

        t_com=torch.cat((input_t,fusion3),dim=1)
        t_com=self.fuse_conv3(t_com)

        rc0,rc1,rc2,rc3=self.Encoder_rgb(rgb_com)
        tc0,tc1,tc2,tc3=self.Encoder_t(t_com)
