...
from model13 import TFNet  ###########################################

times = time.time()
local_time = time.localtime(times)

jilupath= r'/data/LYC/code/dragonNet_content/lyc_result_bise5.txt' ##########################################
target='模型概述:odel9 - model13 使用+融合两分支的细节 +融合三分支语义'   ##############################################
#_______________________________________________________________________________________________________
...
def train(epo, model, train_loader, optimizer, train_dataset):
    ...
    for it, sample in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        pre_loss,fusion_loss = model(RGB_images, T_images) ################################################
        loss1 = cri1(pre_loss, labels)
        loss2 = cri1(fusion_loss, labels)
        loss = loss1+loss2
        loss.backward()
        optimizer.step()

        acc = calculate_accuracy(pre_loss, labels)
        loss_avg += float(loss)
        acc_avg  += float(acc)
        cur_t = time.time()
        if cur_t-t > 5:
            print('|- epo %s/%s. train iter %s/%s. %.2f img/sec loss: %.4f, loss-seg: %.4f, acc: %.4f' \
                % (epo, args.epoch_max, it+1, train_loader.n_iter, (it+1)*args.batch_size/(cur_t-start_t), float(loss), float(loss1), float(acc)))
            t += 5
...
def testing(epo, model, test_loader):
    ...
    with torch.no_grad():
        for it, sample in enumerate(test_loader):
            ...
            prediction = pre_loss.argmax(1).cpu().numpy().squeeze().flatten() # prediction and label are both 1-d array, size: minibatch*640*480
